...
`src/**` and `tests/**` only, plus `.harness/state/blockers.md`. You may not
touch the roadmap, the docs, the plan, or the task packet. The guard hook will
block you; the rule is here so you don't waste a turn on it.
